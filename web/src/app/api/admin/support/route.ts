...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const where: any = {
      type: "SUPPORT",
      OR: [
        { initiator_id: SUPPORT_USER_ID },
        { participant_id: SUPPORT_USER_ID }
      ]
    };
    ...
    // Calculate if conversation needs response (last message not from TOP Support)
    ...
    if (filter === "pending") {
      filteredConversations = formattedConversations.filter(c => c.isPending);
    ...
    const pendingCount = formattedConversations.filter(c => c.isPending).length;
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        answered: formattedConversations.length - pendingCount
      },
      pagination: {
        page,
        limit,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (conversation.type !== "SUPPORT") {
      return NextResponse.json(
        { error: "Not a support conversation" },
        { status: 400 }
      );
    }

    // Determine receiver (the user, not TOP Support)
    const receiverId = conversation.initiator_id === SUPPORT_USER_ID
