...
function kwHits(text: string, keyword: string) {
  ...
  const re = new RegExp(`(^|\\b)${hyphenFlex}(\\b|$)`, "i");
  return re.test(text);
}

function scoreCategoriesWeighted(labelAnnotations: { description?: string|null; score?: number|null }[]) {
  const scores: Record<string, number> = Object.fromEntries(Object.keys(CATEGORY_KEYS).map(k => [k, 0]));
  ...
  if (DEBUG) {
    console.log("labels:", labelAnnotations.map(l => `${l.description}:${(l.score || 0).toFixed(2)}`));
    console.log("scores:", scores);
  }

  const values = Object.values(scores);
  const total = values.reduce((a, b) => a + b, 0);
  if (total === 0) return { category: "Unknown", confidence: 0, scores };
...
export async function OPTIONS() {
  // simple CORS preflight (optional; helpful for web clients)
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // (optional) open CORS for web/demo clients
    res.headers.set("Access-Control-Allow-Origin", "*");
    return res;
  } catch (err: any) {
    const res = NextResponse.json(
