-- Quick fix: Apply gender filter to get_search_feed function
-- This file can be executed directly in Supabase SQL Editor
-- It fixes the gender filtering issue in get_search_feed function

-- Extract only the gender filter part and apply it
-- We'll update the inv_gender_search CTE to include gender filtering
...
BEGIN
  ...
  IF EXISTS (
    ...
    AND p.proname = 'get_search_feed'
  ) THEN
    RAISE NOTICE 'Function get_search_feed exists, proceeding with update...';
  ELSE
    RAISE EXCEPTION 'Function get_search_feed does not exist. Please apply migration 20251110000002_add_search_feed_function first.';
  END IF;
END $$;

-- Now apply the full fixed function
...
BEGIN
  ...
  -- FIX: Added gender filtering - when gender is specified, only return matching items
  inv_gender_search AS (
    ...
    WHERE l.listed = true
      ...
      -- GENDER FILTER: Filter items based on v_gender_norm
      -- Male users: only show Men and Unisex items
      -- Female users: only show Women and Unisex items
      -- Unisex/not set: show all items
      AND (
        v_gender_norm = 'unisex'
        OR l.gender = 'Unisex'
      ...
      -- Search filter: match name, description, brand, tags
      AND (
  ...
  -- Gender match scoring (for personalization)
